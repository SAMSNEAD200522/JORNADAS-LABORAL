...
describe('HolidaysService', () => {
  ...
  describe('create', () => {
    ...
    it('debe crear un festivo exitosamente', async () => {
      mockPrisma.holiday.create.mockResolvedValue({ id: 1, ...dto, date: new Date('2026-07-20') });

      const result = await service.create(dto, 1);
      expect(result).toHaveProperty('id', 1);
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'CREAR', entity: 'Festivo', userId: 1 }),
      );
    });
  ...
  describe('findAll', () => {
    it('debe retornar todos los festivos ordenados por fecha', async () => {
      mockPrisma.holiday.findMany.mockResolvedValue([{ id: 1, date: new Date('2026-01-01'), name: 'Año Nuevo' }]);

      const result = await service.findAll();
      expect(result).toHaveLength(1);
      expect(mockPrisma.holiday.findMany).toHaveBeenCalledWith({ orderBy: { date: 'asc' } });
    });
  });

  describe('findOne', () => {
    it('debe retornar un festivo por id', async () => {
      mockPrisma.holiday.findUnique.mockResolvedValue({ id: 1, date: new Date('2026-07-20'), name: 'Independencia' });
  ...
  describe('remove', () => {
    it('debe eliminar un festivo exitosamente', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'ELIMINAR', entity: 'Festivo', userId: 1 }),
      );
    });
