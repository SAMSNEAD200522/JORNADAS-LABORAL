...
describe('SchedulesService', () => {
  ...
  describe('create', () => {
    ...
    it('debe crear un horario exitosamente', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'CREAR', entity: 'Horario' }),
      );
    });

    it('debe rechazar nombre duplicado (P2002)', async () => {
      const prismaError = Object.assign(new Error('Unique'), {
        code: 'P2002',
        clientVersion: '6.0',
      });
    ...
    it('debe lanzar código HORARIO_DUPLICADO', async () => {
      ...
      try {
        ...
        const response = (e as ConflictException).getResponse() as Record<
          string,
          unknown
        >;
    ...
    it('debe relanzar errores que no son P2002', async () => {
      const otherError = Object.assign(new Error('DB error'), {
        code: 'P1000',
        clientVersion: '6.0',
      });
  ...
  describe('findAll', () => {
    it('debe retornar lista de horarios', async () => {
      mockPrisma.schedule.findMany.mockResolvedValue([
        { id: 1, name: 'Test', _count: { employees: 0 } },
      ]);
  ...
  describe('findOne', () => {
    ...
    it('debe lanzar código HORARIO_NO_ENCONTRADO', async () => {
      ...
      try {
        ...
        const response = (e as NotFoundException).getResponse() as Record<
          string,
          unknown
        >;
  ...
  describe('update', () => {
    it('debe actualizar un horario', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'ACTUALIZAR' }),
      );
    });
    ...
    it('debe rechazar nombre duplicado en update', async () => {
      ...
      await expect(service.update(1, { name: 'Otro' } as any)).rejects.toThrow(
        ConflictException,
      );
    });
  ...
  describe('toggleStatus', () => {
    it('debe alternar de activo a inactivo', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'DESACTIVAR' }),
      );
    });
    ...
    it('debe alternar de inactivo a activo', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'ACTIVAR' }),
      );
    });
  ...
  describe('assignToEmployee', () => {
    it('debe asignar horario a empleado', async () => {
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'ASIGNAR_HORARIO' }),
      );
    });

    it('debe lanzar 404 si el horario no existe', async () => {
      mockPrisma.schedule.findUnique.mockResolvedValue(null);
      await expect(service.assignToEmployee(999, 1)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('debe lanzar 404 si el empleado no existe', async () => {
      ...
      await expect(service.assignToEmployee(1, 999)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('createDay', () => {
    ...
    it('debe crear/actualizar un día del horario', async () => {
      ...
      mockPrisma.scheduleDay.upsert.mockResolvedValue({
        id: 1,
        scheduleId: 1,
        ...dayDto,
      });
      const result = await service.createDay(1, dayDto);
      expect(result).toHaveProperty('scheduleId', 1);
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'CREAR_ACTUALIZAR_DIA_HORARIO' }),
      );
    });

    it('debe rechazar si el horario no existe', async () => {
      mockPrisma.schedule.findUnique.mockResolvedValue(null);
      await expect(service.createDay(999, dayDto as any)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('removeDay', () => {
    it('debe eliminar un día del horario', async () => {
      ...
      mockPrisma.scheduleDay.findUnique.mockResolvedValue({
        scheduleId: 1,
        dayOfWeek: 1,
      });
      ...
      expect(mockAudit.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: 'ELIMINAR_DIA_HORARIO' }),
      );
    });

    it('debe rechazar si el horario no existe', async () => {
      mockPrisma.schedule.findUnique.mockResolvedValue(null);
      await expect(service.removeDay(999, 1)).rejects.toThrow(
        NotFoundException,
      );
    });
